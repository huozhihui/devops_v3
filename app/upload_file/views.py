...
from ..models import Component, UploadFile
from .. import db


@upload_file.route('/index', methods=['GET'])
@login_required
def index():
    header = u'管理'
...
@upload_file.route('/new', methods=['GET', 'POST'])
@login_required
def new():
    header = u'添加'
    form = ComponentForm()
...
@upload_file.route('/edit/<int:id>', methods=['GET', 'POST'])
@login_required
def edit(id):
    header = u"编辑"
...
@upload_file.route('/delete/<int:id>', methods=['GET'])
@login_required
def delete(id):
...
# ajax方式动态增加组件变量
@upload_file.route('/ajax_add_var_tr/<int:i>', methods=['GET', 'POST'])
@login_required
